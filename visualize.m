...
function animate(data, model, thrusts, plots)
    ...
    for t = 1:2:length(data.t)
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%% draw traj %%%%%%%%%%%%%%%%%%%%%%%%%%%%
        hold on;
        scatter3(data.x(1,1:t),data.x(2,1:t),data.x(3,1:t),'MarkerEdgeColor','g',...
        'MarkerFaceColor','g');
        hold off;
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        title('Angular Velocity (r-roll, g-pitch, b-yaw)');

        subplot(plots(3));
        multiplot(data, data.theta, t);
        xlabel('Time (s)');
        ylabel('Angular Displacement (rad)');
        title('Angular Displacement (r-roll, g-pitch, b-yaw)');
    end
end
...
%     h(6) = surf(x, y + 5, z, 'EdgeColor', 'none', 'FaceColor', 'b');
    ...
    length = 2.5*sqrt(2);
    h(3) = surf(x - length, y + length, z, 'EdgeColor', 'none', 'FaceColor', 'b');
    h(4) = surf(x + length, y - length, z, 'EdgeColor', 'none', 'FaceColor', 'b');
    h(5) = surf(x + length, y + length, z, 'EdgeColor', 'none', 'FaceColor', 'b');
    h(6) = surf(x - length, y - length, z, 'EdgeColor', 'none', 'FaceColor', 'b');
...
%     thrusts(4) = surf(x - 5, y, z, 'EdgeColor', 'none', 'FaceColor', 'y');
    ...
    thrusts(1) = surf(x - length, y + length, z, 'EdgeColor', 'none', 'FaceColor', 'm');
    thrusts(2) = surf(x + length, y - length, z, 'EdgeColor', 'none', 'FaceColor', 'm');
    thrusts(3) = surf(x + length, y + length, z, 'EdgeColor', 'none', 'FaceColor', 'k');
    thrusts(4) = surf(x - length, y - length, z, 'EdgeColor', 'none', 'FaceColor', 'k');
